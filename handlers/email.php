...
class email {
    ...
    public static function send($email='',$subject='',$body='',$sendat=null,$headers=[],$queueonly=false) {
        ...
        if(!empty(\db::fetch_row("SELECT id FROM email_queue WHERE email='$email' AND sendat > ".strtotime('-1 minute')." AND sendat < ".strtotime('+1 minute')))) return 1.1;
        $result = \db::insert('email_queue',['email'=>$email, 'subject'=>$subject, 'body'=>$body, 'sendat'=>$sendat, 'headers'=>$headers]);
        if(!$result && !is_array($queueonly)) return self::send($email,$subject,$body,$sendat,$headers,self::database());
        if(!$queueonly) async(function() use ($result){ \email::cron(['id'=>$result]); });
        return $result;
    }
    ...
    public static function cron($data=[], $log=[]) {
        ...
        if(!is_array($fila=\db::fetch_array("SELECT * FROM email_queue  
        ...
        foreach($fila as $item)
            if(!empty($id=($item['id'] ?? ''))) {
                ...
                if(empty($item['email'] ?? '') || empty($item['subject'] ?? '') || empty($item['body'] ?? ''))
                    if(\db::query("UPDATE email_queue SET response='".json_encode(['result'=>'lack parameters'])."' WHERE id='$id' ") > -1) continue;
                //catches the job
                if(\db::query("UPDATE email_queue SET response='".
                    ...
                    WHERE id='$id' AND (response is null OR response='') ") > 0) {
                        ...
                        //applies the answer
                        $log[] = "[$id/email] ".((\db::query("UPDATE email_queue SET response=:r WHERE id='$id' ",[
