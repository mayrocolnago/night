...
class sms {
    ...
    public static function send($number='',$message='',$sendat=null,$sender='',$queueonly=false) {
        ...
        if(!empty(\db::fetch_row("SELECT id FROM sms_queue WHERE (receipt='$number' or receipt='55$number') AND sendat > ".strtotime('-1 minute')." AND sendat < ".strtotime('+1 minute')))) return 1.1;
        $result = \db::insert('sms_queue',['receipt'=>$number, 'sender'=>$sender, 'message'=>$message, 'sendat'=>$sendat]);
        if(!$result && !is_array($queueonly)) return self::send($number,$message,$sendat,$sender,self::database());
        if(!$queueonly) async(function() use ($result){ \sms::cron(['id'=>$result]); });
        return $result;
    }
    ...
    public static function cron($data=[], $log=[]) {
        ...
        if(!is_array($fila=\db::fetch_array("SELECT * FROM sms_queue  
        ...
        foreach($fila as $item)
            if(!empty($id=($item['id'] ?? ''))) {
                ...
                if(empty($item['receipt'] ?? '') || empty($item['message'] ?? ''))
                    if(\db::query("UPDATE sms_queue SET response='".json_encode(['result'=>'lack parameters'])."' WHERE id='$id' ") > -1) continue;
                //catch the job to execute
                if(\db::query("UPDATE sms_queue SET response='".
                    ...
                    WHERE id='$id' AND (response is null OR response='') ") > 0) {
                        ...
                        $log[] = "[$id/sms] ".((\db::query("UPDATE sms_queue SET response=:r WHERE id='$id' ",[
