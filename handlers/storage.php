...
class storage {
    ...
    protected static function database() {
        \db::query("CREATE TABLE IF NOT EXISTS `vault_hashes` (
        ...
        \db::query("CREATE TABLE IF NOT EXISTS `vault_files` (
            `id` bigint(20) NOT NULL AUTO_INCREMENT,
    ...
    public static function storefile($filename, $content='', $tags=[]) {
        ...
        if(\db::query("UPDATE vault_files SET lastchange=:lc WHERE filepath=:fp 
                ORDER BY id DESC LIMIT 1",['lc'=>$strtotime, 'fp'=>"/$filename"]) < 1)
           \db::insert("vault_files",[
               'filepath' => "/$filename",
               'hashcheck' => $checksum,
               'lastseen' => '0',
               'lastchange' => '0',
        ...
        if(!empty($tags['redirect'] ?? '')) return $filename;
          ...
          if(\db::query("UPDATE vault_hashes SET lastcheck=:lc WHERE hashcheck=:hc order by id desc limit 1",[
                'lc' => $strtotime, 'hc' => $checksum ]) > 0 && self::updatefiletags($checksum))
            return $filename;
          else
            if(\db::insert("vault_hashes",[
                'hashcheck' => $checksum,
                'content' => $content,
                'lastcheck' => '0',
    ...
    private static function updatefiletags($hash,$params=[]) {
        ...
        \db::query("UPDATE vault_files SET tags=:tg WHERE hashcheck=:hc order by id desc limit 1",[ 'tg'=>$params, 'hc'=>$hash ]);
        return true;
    }
    ...
    public static function get_contents($nameurl='',$details=false) {
        ...
        return ("data://application/octet-stream;base64,".base64_encode(
            \db::fetch_item("SELECT content c FROM vault_hashes WHERE hashcheck=:hc ORDER BY id DESC LIMIT 1",[
                'hc' => $hash ])['c'] ?? ''));
    }
    ...
    public static function __callStatic($name='',$arg=[]):\route {
        ...
        $hash = (($file = \db::fetch_item("SELECT id,hashcheck,tags 
        ...
        if(!empty($file['id'] ?? 0))
            if(\db::query("UPDATE vault_files SET lastseen=:ls WHERE id=:vd LIMIT 1",['ls'=>strtotime('now'), 'vd'=>$file['id']]) || true)
                if(!empty($file['tags']['redirect'] ?? '')) exit(@header('Location: '.$file['tags']['redirect']));
                else if(!empty($id = ($vault = \db::fetch_item("SELECT id,content FROM vault_hashes WHERE hashcheck=:hs ORDER BY id DESC LIMIT 1",['hs'=>$hash]))['id'] ?? '')) {
