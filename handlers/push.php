...
class push {
    ...
    public static function send($to=null,$body=null,$msg=null,$title=null,$send_at=null,$tags=null,$queueonly=false) {
        ...
        if(!$queueonly) async(function() use ($result){ \push::cron(['id'=>$result]); });
        return $result;
    }
    ...
    public static function cron($data=[], $log=[]) {
        ...
        if(!is_array($fila=\db::fetch_array("SELECT * FROM push_queue  
        ...
        foreach($fila as $item)
            if(!empty($id=($item['id'] ?? ''))) {
                ...
                if(empty($item['mensagem'] ?? ''))
                    if(\db::query("UPDATE push_queue SET response='".json_encode(['result'=>'lack parameters'])."' WHERE id='$id' ") > -1) continue;
                //catches the job
                if(\db::query("UPDATE push_queue SET response='".
                        json_encode(['result'=>'sending', 'date'=>date('Y-m-d H:i:s')])."'
                    WHERE id='$id' AND (response is null OR response='') ") > 0) {
                        //log sending
                        $log[] = "[$id/push] sending...";
                        //applies the answer
                        $log[] = "[$id/push] ".((\db::query("UPDATE push_queue SET response=:r WHERE id='$id' ",[
