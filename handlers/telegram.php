...
class telegram {
    ...
    public static function database() {
        return \db::create("telegram_queue",[
    ...
    public static function cron($data=[], $log=[]) {
        ...
        if(!is_array($fila=\db::fetch_array("SELECT * FROM telegram_queue  
        ...
        foreach($fila as $item)
            if(!empty($id=($item['id'] ?? ''))) { sleep(1);
                ...
                if(!\db::query("UPDATE telegram_queue SET response='".json_encode(['result'=>'processing'])."' WHERE id='$id' AND (response is null or response='')")) continue;
                ...
                if(empty($item['botkey']) || empty($item['params']['chat_id']))
                    if(\db::query("UPDATE telegram_queue SET response='".json_encode(['result'=>'lack parameters'])."' WHERE id='$id' ") > -1) continue;
                //catches the job to do
                if(\db::query("UPDATE telegram_queue SET response='".
                        json_encode(['result'=>'sending', 'date'=>date('Y-m-d H:i:s')])."'
                    WHERE id='".$item['id']."' ") > 0) {
            
                        $log[] = "[$id/telegram] sending to ****".substr($item['params']['chat_id'],-4)."...";
                        
                        $log[] = "[$id/telegram] ".((\db::query("UPDATE telegram_queue SET response=:r WHERE id='$id' ",[
    ...
    public static function send($message='',$chatid=null,$method='sendMessage',$params=[],$sendat=null,$queueonly=false) {
        ...
        if(!empty($lastid = (\db::fetch_row("SELECT id FROM telegram_queue ORDER BY id DESC LIMIT 1")['id'] ?? null))) $lastid++;
        $result = \db::insert("telegram_queue", ['id'=>$lastid, 'message'=>rmAentities($message ?? ''), 'chatid'=>$chatid, 'method'=>$method, 'params'=>$params, 'sendat'=>$sendat]);
        if(!$result && !is_array($queueonly)) return self::send($message,$chatid,$method,$params,$sendat,self::database());
        if(!$queueonly) async(function() use ($result){ \telegram::cron(['id'=>$result]); });
        return $result;
    }
