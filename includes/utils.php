...
if(!function_exists('curlsend')) {
  ...
  function utils_database() {
      return \db::create("global_configs",[
  ...
  /* list modules */
  function listmodules($path = (REPODIR.'/resources/'), &$modules=[]) {
  ...
  /* JWT hash generator */
  function jwt_encode($payload=[], $header=512, $secret=null) {
  ...
  /* JWT decoder */
  function jwt_decode($hash='', $secret=null) {
    ...
    if($hash !== jwt_encode($payload, $header, $secret)) return [];
  ...
  function useragent($includepush=false) {
  ...
  function setconfig($key=null,$value=null) {
    ...
    if(!isset($_SERVER['configs'])) getconfig();
    if(($_SERVER['configs'][$key] = $value) === null
    &&(is_numeric(\db::query("DELETE FROM global_configs WHERE ckey='$key'")))) return $value;
    if(is_array($value) || is_object($value)) $value = json_encode($value);
    if(is_numeric($value) && $value === "0") $value = false;
    if(is_string($value)) $value = emojientities($value);
    if(is_bool($value)) $value = (($value) ? "1" : "0");
    utils_database(); $now = strtotime('now');
    if(\db::query("UPDATE global_configs SET cvalue=:v, updated_at=:u WHERE ckey=:c",['v'=>$value, 'u'=>$now, 'c'=>$key])
    ||(\db::insert('global_configs',['ckey'=>$key, 'cvalue'=>$value, 'updated_at'=>$now])))
      return $_SERVER['configs'][$key];
    return null;
  }

  function getconfig($key=null,$def=null,$savedef=true) {
    if(is_string($key) && empty($key = preg_replace('/[^0-9a-zA-Z\_\-]/','',$key))) return null;
    if(!isset($_SERVER['configs']) && is_array($_SERVER['configs'] = []) && is_array($db = \db::fetch_array("SELECT * FROM global_configs")))
    ...
    if(!empty($def) && empty($_SERVER['configs'][$key] ?? null) && $savedef) setconfig($key,$def);
  ...
  function savecookie($key,$cvalue='',$tempo='auto') {
  ...
  function remainingstr($value1,$value2='',$format='{dD} {hH} {iI} {sS}') {
  ...
  function datetostrtotime($date,$dmyonly=true) {
  ...
  function str_maskmiddle($input='',$above=0) {
  ...
  function str_maskmiddle_array($array=[],$onlykeys=[],$above=0) {
    if(is_array($array))
  ...
  /* names in portuguese */
  function ucstrname($string, $delimiters = array(" ", "-", ".", "'", "O'", "D'", "Mc"), $exceptions = array("de", "da", "dos", "das", "do", "I", "II", "III", "IV", "V", "VI", "LTDA", "ME", "MEI")) {
  ...
  function floatval_safe($number,$decimals=2) {
  ...
  function rmAentities($string) {
    return str_replace(['&lt;','&gt;','&quot;','&amp;','&nbsp;'],['<','>','"','&',' '],htmlentities($string));
  }

  /* shows my ip */
  function remote_addr($default='0.0.0.0') {
  ...
  function html_tag_attrib($tag="a", $attribute="href", $html="") {
  ...
  function html_tag_content($tag="a", $html="") {
    $pattern = '/<' . $tag . '[^>]*>(.*?)<\/' . $tag . '>/is';
    preg_match($pattern, $html, $matches);
    if(!empty($matches[1] ?? '')) return $matches[1];
    else return '';
        ...
        if(typeof cachectl !== 'boolean' || cachectl === 'store' || cachectl === 'cache') {
          ...
          if(typeof cachectl === 'string')
            if(empty(cachestorage[cachecode])) cachectl = {};
              ...
              if(typeof cachestorage[cachecode] !== 'object') cachestorage[cachecode] = {};
              else {
