...
abstract class auth {
    ...
    private function database($tablename=null) {
        ...
        return \db::create(($tablename ?? $this->table),$fields,"id",$uniques);
    }
    ...
    private function get($data=null,$id=null,$mask=null):\route {
        ...
        if(empty($user = ($this->cache["userdata_$id".(($mask)?"_masked":"_unmasked")] ?? []))) {
            ...
            if(empty($info = \db::fetch_row("SELECT * FROM {$this->table} WHERE id='$id' and active='1' LIMIT 1"))) return response()->data(0);
    ...
    private function recovery($data=[], &$key=null):\route {
        ...
        if(empty($key))
            ...
            else if(!is_array($user = \db::fetch_row("SELECT id, passw FROM {$this->table} WHERE login=:u AND active='1' LIMIT 1",['u'=>$login]))) return response()->data(-2);
        ...
        if(!is_array($user = \db::fetch_row("SELECT passw FROM {$this->table} WHERE id=:id AND active='1' LIMIT 1",['id'=>$id]))) return response()->data(-9);
    ...
    private function validate($data=[], $user=null, $secret=null, $api=null) {
        ...
        if(empty($user) && (!empty($userid = $this->id())))
            if((!empty($get = \db::fetch_row("SELECT * FROM {$this->table} WHERE id=:id LIMIT 1",['id'=>$userid]))) && is_array($user = $get))
    ...
    private function signup($user=[],&$passunhashed=null):\route {
        ...
        return response()->data([
            'result' => intval($id = \db::insert($this->table,$user)), 
    ...
    private function signin($data=[]):\route {
        ...
        if(empty($id = intval(($db = \db::fetch_row("SELECT id, login, info ".(($this->lockRetries ?? false) ? ",lockpenalty " : "")."
        ...
        if($this->lockRetries ?? false) {
            ...
            if(empty($currentlock = @intval(substr(($db['lockpenalty'] ?? '0'),-12))) || ($currentlock < $retrystart)) {
                ...
                \db::query("UPDATE {$this->table} SET lockpenalty=(COALESCE(lockpenalty,0) + $diference) WHERE id='$id' AND COALESCE(lockpenalty,0) < $maxpunishment"); }
            //Now we are always adding a punishment for ensurance
            \db::query("UPDATE {$this->table} SET lockpenalty=(lockpenalty + $lockincrement) WHERE id='$id' AND COALESCE(lockpenalty,0) < $maxpunishment");
        ...
        if(empty($phrs = (($pa = \db::fetch_row("SELECT passw ".(($this->lockRetries ?? false) ? ",lockpenalty " : "")."
    ...
    private function signout($data=null,$connection=null,$id=null):\route {
        ...
        return $redirect($r,intval(\db::query("UPDATE {$this->table} SET lastseen=:ls ,devices=:dv WHERE id='$id' ", ['ls'=>$now, 'dv'=>$new])) > 0);
    }
    ...
    private function id(&$token=null) {
        ...
        if($this->allowAPIaccess ?? false)
            if(!is_numeric(explode('_',($token."_"))[0] ?? ''))
                if(!empty($aid = intval(\db::fetch_row("SELECT id FROM {$this->table} 
        ...
        if(empty($user = \db::fetch_row("SELECT passw, devices FROM {$this->table} WHERE id=:id AND active='1' LIMIT 1",['id'=>$id]))) return ($this->cache['userid'] = 0);
    ...
    private function token($id=0) {
        ...
        //Save data to user
        return ((\db::query("UPDATE {$this->table} SET lastseen=:ls ,devices=:dv WHERE id='$id' ", ['ls'=>$now, 'dv'=>$devices]))
            ? $token : null);
    }
    ...
    private function getOTP($for=null, $restricted=true, $margin=null, $timestamp=null) {
        ...
        if($restricted) {
            ...
            if(intval($lastotp = (\db::fetch_row("SELECT requested_at as req FROM otpcodes WHERE keyname=:id
            ...
            if(\db::query("UPDATE otpcodes SET requested_at=:req WHERE keyname=:id", ['req'=>strtotime($this->requestOTPinterval), 'id'=>$for]) < 1)
                if(\db::query("INSERT INTO otpcodes (keyname,requested_at) VALUES (:id,:req)", ['id'=>$for, 'req'=>strtotime($this->requestOTPinterval)]) < 1)
                    $this->database(); }
    ...
    private function enable2FA($data=[]):\route {
        ...
        if(empty($user = \db::fetch_row("SELECT id,tags FROM {$this->table} WHERE id=:id LIMIT 1", ['id'=>$userid]))) return response()->data(-405);
        if(!is_array($user['tags'] ?? '')) $user['tags'] = []; $user['tags']['2fasecret'] = $secret;
        return response()->data(\db::query("UPDATE {$this->table} SET tags=:tags WHERE id=:id LIMIT 1", ['tags'=>$user['tags'], 'id'=>$userid]));
    }
    ...
    private function has2FA($data=[]):\route {
        ...
        if(empty($user = \db::fetch_row("SELECT tags FROM {$this->table} WHERE $field=:id LIMIT 1", ['id'=>$search]))) return response()->data(false);
    ...
    private function has($permission='admin',$id=null) {
        ...
        if(!is_array($p = ($this->get('permission',$id)->data ?? '')))
            if(!is_array($p = explode(',',','.(\db::fetch_row("SELECT permission FROM {$this->table} WHERE id='$id' LIMIT 1")['permission'] ?? '')))) return false;
    ...
    private function add($permission='permission',$id=null) {
        ...
        return (\db::query("UPDATE {$this->table} SET permission='$p' WHERE id='$id' LIMIT 1") > 0);
    }
    ...
    private function exists($data=[]):\route {
        ...
        return response()->data(!empty(\db::fetch_row("SELECT id 
            FROM {$this->table}
