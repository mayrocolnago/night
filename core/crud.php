...
trait crud {
    ...
    //Optionally implement crudCompletionHandler($data, $action, $table, $result){ return true; }; function on your class
    ...
    public static function create($data=[]):\route {
        ...
        $result = intval(\db::insert($table, $values));
    ...
    public static function list($data=null,$order=null,$limit=100,$from=null):\route {
        ...
        //returns
        $result = (\db::fetch_array("SELECT * FROM $table ".((empty($where))?"":"WHERE ".
    ...
    public static function update($data=[]):\route {
        ...
        else $result = intval(\db::query("UPDATE $table SET ".implode(', ',array_merge_recursive(
    ...
    public static function delete($data=[]):\route {
        ...
        //returns
        if(empty($where)) $result = 0;
        else $result = intval(\db::query("DELETE FROM $table WHERE ".
            preg_replace('/^(OR |AND )|(OR |AND )$/', '', implode(" ", array_map(function($a){
                return (((strpos($a,'|') !== false) ? "OR " : "AND ").str_replace(['|','!','^','~'],'',$a).
                        ((strpos($a,'!') !== false && (!(strpos($a,'^') !== false))) ? " NOT" : "").
                        ((strpos($a,'^') !== false) ? " IS ".((strpos($a,'!') !== false) ? "NOT " : "") : " LIKE ")."?");
